...
class TestUser(Initializer):
    def test_create_new(self):
        resp = self.client.post('users', json={'name': 'Jon Doe'})
        assert resp.status_code == 201
        assert resp.json == {'user_id': 1}

        resp = self.client.post('users', json={'name': 'Rostyslav Bilan'})
        assert resp.status_code == 201
    ...
    def test_successful_get_user(self):
        ...
        assert resp.json == {'name': 'Jon Doe'}

    def test_get_unexistent_user(self):
        resp = self.client.get(f'/users/1')
        assert resp.status_code == 404
        assert resp.json == {'error': f'No such user_id 1'}
